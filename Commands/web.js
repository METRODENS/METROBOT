...
let {sendMessagecalculate} = require('metro-bot-pro/sources/dc/cmd/cal')
...
MetroBot['IntroduceCMD']({pattern: 'speedtest ?(.*)', fromMe: true, desc: DATA.SPEEDTEST_DESC, usage: 'speedtest user // speedtest server'}, (async (message, input) => {
...
MetroBot['IntroduceCMD']({pattern: 'ping$', fromMe: true, delownsewcmd: false, desc: DATA.PING_DESC}, (async (message, input) => {
  var start = new Date().getTime();
  ...
  await message.client.sendMessage(
    ...
    MetroBot['IntroduceCMD']({pattern: 'short ?(.*)', fromMe: WorkType, desc: DATA.URL}, (async (message, input) => {
    ...
    MetroBot['IntroduceCMD']({pattern: 'cal ?(.*)', fromMe: WorkType, desc: 'Powerfull Calculater With >  - + * / × ÷ π e ( ) pie pi log lg ^ 1-9'}, (async (message, input) => {
       var textsew = '';
